...
public:
    ...
    void check()
    {
        ...
        else if(this->get_clock()->now().toSec() - last_time > silent_limit)
        {
            isalive.data = false;
            ROS_WARN("[%s] has been silient for: [%f] second."
                ,name, this->get_clock()->now().toSec() - last_time);
        }
        else
        {
            isalive.data = true;
...
{
public:
    Imu(rclcpp::Node::SharedPtr node n,bool isdebug):Sensor(isdebug)
    {
        strcpy(name, "imu");
    ...
    {
        last_time = this->get_clock()->now().toSec();
...
{
public:
    Odom(rclcpp::Node::SharedPtr node n,bool isdebug):Sensor(isdebug)
    {
        strcpy(name, "odom");
...
{
public:
    Odom2(rclcpp::Node::SharedPtr node n,bool isdebug):Sensor(isdebug)
    {
        strcpy(name, "odom2");
...
{
public:
    Joint(rclcpp::Node::SharedPtr node n,bool isdebug):Sensor(isdebug)
    {
        strcpy(name, "joint");
...
{
public:
    Realsense(rclcpp::Node::SharedPtr node n,bool isdebug):Sensor(isdebug)
    {
...
{
public:
    Lidar(rclcpp::Node::SharedPtr node n,bool isdebug):Sensor(isdebug)
    {
        strcpy(name, "lidar");
...
{
public:
    Ultrasound(rclcpp::Node::SharedPtr node n,bool isdebug):Sensor(isdebug)
    {
...
public:
    ...
    SensorChecker(rclcpp::Node::SharedPtr node n, bool isdebug) : 
...
int main(int argc, char **argv)
{
    ...
    //program init
    rclcpp::init(argc, argv, "sensor_checker");  
    rclcpp::Node::SharedPtr node n;
    bool isdebug;
    ...
    //loop
    rclcpp::Rate loop_rate(sensor_checker.check_freq_hz);
    while (rclcpp::ok())
    {
        if(isdebug)  {ROS_INFO("time now is: [%f]",this->get_clock()->now().toSec());}

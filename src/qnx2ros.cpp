...
class QNX2ROS : public rclcpp::Node {
  public:
    QNX2ROS(int server_port): Node("udp2ros")
    {
      ...
      // leg_odom_pub_ = this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("leg_odom", 1);    
      // leg_odom_pub2_ = this->create_publisher<nav_msgs::msg::Odometry>("leg_odom2", 1);
      joint_state_pub_ = this->create_publisher<sensor_msgs::msg::JointState>("/joint_states", 1);
      // imu_pub_100_ = nthis->create_publisher<sensor_msgs::Imu>("/imu/data", 1);
      imu_pub_200_ = this->create_publisher<sensor_msgs::msg::Imu>("/imu/data", 1);    
      handle_pub_ = this->create_publisher<geometry_msgs::msg::Twist>("/handle_state", 1);
      // ultrasound_pub_ = this->create_publisher<std_msgs::msg::Float64>("/us_publisher/ultrasound_distance", 1);
    }
  ...
  void ParseRobotStateFrame(){
    ...
    if (dr->code == 2305) 
    {
      ...
      // leg_odom_data.header.stamp = this->get_clock()->now();
      ...
      // leg_odom_data2.header.stamp = this->get_clock()->now();
      ...
      // leg_odom_trans.header.stamp = this->get_clock()->now();
  ...
  void ParseJointStateFrame(){
    ...
    if (dr->code == 2306)
    {
      ...
      joint_state_data.header.stamp = this->get_clock()->now();
...
private:
  ...
  //rclcpp::Subscription<std_msgs::msg::UInt8>::SharedPtr Publisher ultrasound_pub_;
...
int main(int argc, char **argv) {
  ...
  int server_port = 43897;
  auto node = std::make_shared<QNX2ROS>(server_port);
